...
The current Ractive instance. It's the same value as `this` inside lifecycle events, observers, event handlers and methods, etc. Thus you have access to the instance's properties and methods from the template.
...
For arrays, `@key`'s value will be the iteration index, similar to `@index`.
